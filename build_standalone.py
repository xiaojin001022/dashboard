...
<div class="sidebar">
  ...
  <button class="btn btn-sync" onclick="triggerSync()" id="btnSync">🔄 同步最新数据</button>
...
function triggerSync() {
  ...
  if (isGitHubPages) {
    // GitHub Pages: 打开 Actions 页面手动触发
    window.open('https://github.com/xiaojin001022/dashboard/actions/workflows/sync.yml', '_blank');
  } else {
    // 本地环境: 调用本地服务器同步
    syncFromTencentDoc();
  }
...
(async function init() {
  ...
  if (isGitHubPages) {
    // GitHub Pages: 使用内嵌数据，每5分钟自动同步（由 GitHub Actions 驱动）
    ALL_DATA = EMBEDDED_DATA;
    serverAvailable = false;
    setStatus('数据每5分钟自动同步（GitHub Actions）<br><small>点击上方按钮可手动触发立即同步</small>', true);
    buildNav();
    renderAll();
    showSection(curSec);
  } else if (isLocal) {
...
function renderChangeover(){
  ...
  // 分离汇总和普通线体（总切线次数 = 各线体之和，不读源数据"汇总"单元格，避免源数据未同步导致总计不更新）
  var lineNames=[],values=[],totalVal=0;
  for(var i=0;i<allNames.length;i++){
    if(allNames[i]==='汇总'){continue;}
    lineNames.push(allNames[i]);
    var v=allVals[i]||0;
    values.push(v);
    totalVal+=v;
  }
